...
app.controller("flowyController", function ($scope, $timeout, $http) {
  
  $scope.loading = true;
  $scope.saving = false;
  $scope.elements = [];
  $scope.roomCreated = false;
  var roomInfo = 'Room created. Link: '+ window.location.protocol + '//' + window.location.host + '/collab/';
  $scope.roomData = '';
  ...
  $scope.createCollab = function(empty){
    var data = {};
    if(!empty){
      data.list = $scope.elements;
    }
    $http.post('/createCollab', data).then(function(response, status){
      $scope.roomData = roomInfo + response.data.room;
  ...
  $scope.updateRemote = function(){
    $scope.saving = true;
    $http.post('/updateData', {list: $scope.elements}).then(function(response, status){
      if(response.data.list){
        $scope.elements = response.data.list;
      }
      $scope.loading = false;
